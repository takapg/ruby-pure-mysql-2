...
RSpec.shared_examples 'a MySQL-compatible server' do |port|
  ...
  describe 'Built-in Functions support' do
    ...
    describe 'LPAD / RPAD function support' do
      ...
      it 'truncates string if length is shorter than string (LPAD/RPAD)' do
        expect(client.query('SELECT LPAD("hi", 1, "??");').first.values.first).to eq('h')
        expect(client.query('SELECT RPAD("hi", 1, "??");').first.values.first).to eq('h')
      end
      ...
      it 'returns NULL if any argument is NULL' do
        expect(client.query('SELECT LPAD(NULL, 5, "a");').first.values.first).to be_nil
        expect(client.query('SELECT LPAD("hi", NULL, "a");').first.values.first).to be_nil
      ...
      it 'returns an error for invalid number of arguments' do
        ...
        expect { client.query('SELECT RPAD("hi", 5, "a", "b");') }.to raise_error(Mysql2::Error)
      end
    end
