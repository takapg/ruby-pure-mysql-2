...
RSpec.describe RubyPureMysql::StorageEngine do
  ...
  describe 'NULL値の厳格な検証 (MySQL 8.0 互換)' do
    ...
    it 'IS NULL 演算子を使用した場合に、正しく NULL 行が抽出されること' do
      ...
      # val <=> 10 -> [2] (10である行のみ)
      where_null_safe_val = [{ column: 'val', operator: '<=>', value: 10 }]
      expect(engine.find_matching_indices(nil, rows, cols, where_null_safe_val)).to contain_exactly(1)
